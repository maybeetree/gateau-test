 ...
 */
__host__ inline 
float get_jn_noise(float T, float nu) 
{
        ...
        )
{
    ...
    if (idx < cnt && idy < f_src.num) 
    {
        ...
        eta_atm_interp = __powf(
                eta_atm_interp, 
                csc_el_shared[threadIdx.x]
                );
        ...
        for (int n=0; n<cnum_stage; n++) 
        {
            ...
            if (psd_parasitic_use < 0) {psd_parasitic_use = psd_sky;}
        ...
        for(int k=0; k<cnf_ch; k++) 
        {
            ...
            atomicAdd(
                    &nepout[k*cnt + idx], 
                    sigfactor * (HP * freq + psd_in_k + cGR_factor)
                    ); 
        }
    }
}
...
{    
    ...
    if (idx < cnt) 
    {
        ...
        for(int k=0; k<cnf_ch; k++) 
        {
            ...
            sigout[k*cnt + idx] = c0[k] + c1[k] * P_k;
        }
        ...
        ) 
{
    ...
    const char *power_str = "P";
    const char *tb_str = "Tb";

    readEtaATM<float, ArrSpec>(
            &eta_atm, 
            &pwv_atm, 
            &f_atm, 
            atmpath
    ...
    gpuErrchk( 
            cudaMalloc(
                (void**)&d_eta_cascade, 
                nf_src * (cascade->num_stage + 1) * sizeof(float)
                ) 
            );
    gpuErrchk( 
            cudaMemcpy(
    ...
    curandState *devstates, *devstates_pink;
    gpuErrchk( 
            cudaMalloc(
                (void**)&devstates, 
                ntscr * sizeof(curandState)
                ) 
            );
    ...
    init_random_states<<<gridSize1D, blockSize1D>>>(
            devstates, 
            seed,
            ntscr
            );
